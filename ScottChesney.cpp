...
vector<double> ScottChesney::generateRiskNeutralPricePath(double toDate, int nSteps, std::vector<double>& vol_path, std::vector<double>& spot_draws) const{
    vector<double> spot_path(nSteps,0.0);
    spot_path[0] = getStockPrice();
    double dt = (toDate-getDate())/nSteps;
    double sqrt_dt = sqrt(dt);
...
vector<double> ScottChesney::calc_vol_path(double toDate, int nSteps, const std::vector<double> &vol_draws) const{
    ...
    for (int i=1; i<nSteps; i++) {
      ...
      vol_path[i] = std::max(vol_path[i-1] + kappa * (theta - vol_path[i-1]) * dt + xi * sqrt(dt) * vol_draws[i-1], 0.0);
    }
